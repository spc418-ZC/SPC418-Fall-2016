...
int main(int argc, char **argv)
{
  ...
  ros::Publisher FrontRightMotorCMD = DiffDriveNode.advertise<std_msgs::Float64>("front_right_controller/command",1);
  ros::Publisher FrontLeftMotorCMD  = DiffDriveNode.advertise<std_msgs::Float64>("front_left_controller/command",1);
  ros::Publisher BackLeftMotorCMD   = DiffDriveNode.advertise<std_msgs::Float64>("back_left_controller/command",1);
  ros::Publisher BackRightMotorCMD  = DiffDriveNode.advertise<std_msgs::Float64>("back_right_controller/command",1);
  ...
  while(ros::ok())
  {
    ...
    FrontRightMotorCMD.publish(right_wheel_vel);
    BackRightMotorCMD.publish(right_wheel_vel);
    FrontLeftMotorCMD.publish(left_wheel_vel);
    BackLeftMotorCMD.publish(left_wheel_vel);
